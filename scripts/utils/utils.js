...
export class colorUtils {
	static rgb(pColor) {
		if (typeof colorUtils == "string") {
			...
			if (!(vColor[0] == "#")) {
				vColor = colorUtils.cStupidList[vColor];
			}
			
			let vRGB = {
				r : parseInt(vColor.slice(1, 3), 16),
				g : parseInt(vColor.slice(3, 5), 16),
				b : parseInt(vColor.slice(5, 7), 16)
			}
	...
	static colorDistance(pColorA, pColorB) {
		let vRGBA = colorUtils.rgb(pColorA);
		let vRGBB = colorUtils.rgb(pColorB);
		
		let vDiffs = [];
		
		for (let vKey of ["r", "g", "b"]) {
			vDiffs.push(vRGBA[vKey] - vRGBB[vKey]);
		}
		
		return Math.sqrt(Math.hypot(...vDiffs));
	}
	
	static get cStupidList() {
